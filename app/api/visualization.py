...
pre{margin:0;background:#0f1116;padding:12px;border-radius:10px;overflow:auto;max-height:340px;font:12px/1.45 ui-monospace,SFMono-Regular,Menlo,monospace}.hidden{display:none}.review-actions{display:flex;gap:8px;flex-wrap:wrap;margin:10px 0}.review-note{color:#9da6b3;font-size:12px;margin-top:5px}
...
<div id="app" class="hidden">
  ...
  <div class="panel">
    <div class="phase-title"><div><strong>Cycle map</strong><div style="color:#9da6b3;font-size:13px;margin-top:3px">Each segment shows an inferred active phase and its approaches.</div></div><div class="badge">Ground truth: unavailable</div></div>
  ...
  <div class="bottom">
    ...
    <div class="panel validation"><strong>How to judge the result</strong><p><strong>1.</strong> GREEN/YELLOW should match the active approaches of the phase.</p><p><strong>2.</strong> Active approaches should show recurring release activity around the same cycle position.</p><p><strong>3.</strong> Inactive approaches should show more waiting/queue evidence around RED periods.</p><p class="warn"><strong>Important:</strong> the source JSON has no measured traffic-light state, so this is a consistency check, not proof of controller correctness.</p></div>
  </div>
  <div class="panel"><strong>Review log for manual checking</strong><div class="review-note">Copy or download this log and send it to me. I will review the cycle, phase assignments, state transitions and traffic evidence.</div><div class="review-actions"><button id="copyLog" class="btn secondary" disabled>Copy review log</button><button id="downloadLog" class="btn secondary" disabled>Download review.log</button></div><pre id="reviewLog">—</pre></div>
...
function buildCycleMap(){const map=$('cycleMap');map.innerHTML='';for(const p of payload.phase_model.phases){const width=Math.max(5,((p.phase_end-p.phase_start+payload.cycle.estimated_seconds)%payload.cycle.estimated_seconds)/payload.cycle.estimated_seconds*100);const d=document.createElement('div');d.className='phase-segment';d.style.width=width+'%';d.innerHTML=`<b>Phase ${p.phase_id}</b><small>${p.phase_start.toFixed(0)}–${p.phase_end.toFixed(0)} s · ${(p.active_approaches||[]).join('/')||'—'}</small>`;map.appendChild(d)}}
